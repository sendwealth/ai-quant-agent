...
class ScreeningEngine:
    ...
    def screen(
        ...
        try:
            ...
            _names = (
                universe_df["name"].tolist() if "name" in universe_df.columns else [""] * len(_syms)
            )
            universe_df = universe_df.assign(
                name=[n or STOCK_NAME_MAP.get(c, "") for c, n in zip(_syms, _names, strict=False)]
            )

            # 2. Name-based pre-filter
            universe_df = self._prefilter.filter_universe(universe_df)
            codes = universe_df["symbol"].tolist()
            name_map = dict(zip(universe_df["symbol"], universe_df["name"], strict=False))
    ...
    def _score_stock(
        ...
        total = tech["technical"] + mom["momentum"] + liq["liquidity"] + fun["fundamental"]

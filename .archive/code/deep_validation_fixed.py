...
def objective(params, data):
    ...
    try:
        ...
        # 确保指标计算完成
        data_copy['sma_short'] = data_copy['sma_short'].ffill()
        data_copy['sma_long'] = data_copy['sma_long'].ffill()
        ...
        # 均线金叉买入
        data_copy.loc[data_copy['sma_short'] > data_copy['sma_long'], 'signal'] = 1
        # 均线死叉卖出
        data_copy.loc[data_copy['sma_short'] < data_copy['sma_long'], 'signal'] = -1
        ...
        # 最终平仓
        if position > 0:
            capital = position * data_copy['close'].iloc[-1]

        if len(equity_curve) < 50 or trades < 5:
            return -999  # 数据不足或交易太少
...
def genetic_algorithm(data, pop_size=30, generations=50, mutation_rate=0.1):
    ...
    for gen in range(generations):
        ...
        for params in population:
            ...
            # 更新最优
            if score < best_score and score > -999:
                best_score = score
                best_params = params

        # 进度
        if gen % 10 == 0:
            current_best = min([s for s in scores if s > -999], default=999)
            print(f"  第{gen}代: 最优夏普比率 = {-current_best:.2f}")

        # 选择（轮盘赌）
        valid_scores = [s if s > -999 else float('inf') for s in scores]
        min_score = min(valid_scores)
        max_score = max(valid_scores)
        
        if max_score - min_score > 0:
            weights = [(s - min_score) / (max_score - min_score) if s > -999 else 0 for s in scores]
            total_weight = sum(weights)
            if total_weight > 0 and not np.isnan(total_weight):
                weights = [w / total_weight for w in weights]
            else:
                weights = [1.0 / pop_size] * pop_size
        else:
            weights = [1.0 / pop_size] * pop_size
        ...
        if abs(total_weight - 1.0) > 0.01:
            weights = [w / total_weight if total_weight > 0 else 1.0/pop_size for w in weights]
...
if completed == total:
    ...
    if results.get('optimize_params'):
        print(f"  ✓ 找到最优参数: MA({ma_short}/{ma_long})")
        print(f"  ✓ 优化后夏普比率: {sharpe_ratio:.2f}")
        if total_return > 0:
            print(f"  ✓ 总收益: {total_return*100:.2f}%")
        else:
            print(f"  ⚠️  总收益: {total_return*100:.2f}%")

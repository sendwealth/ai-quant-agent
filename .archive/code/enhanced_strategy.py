...
MAX_DAILY_LOSS = -0.03  # 最大日亏损比例（-3%）
VOLUME_MA_PERIOD = 20
VOLUME_MULTIPLIER = 1.2  # 成交量确认倍数
ADX_PERIOD = 14
ADX_THRESHOLD = 25  # ADX趋势阈值
SIGNAL_CONFIRM_DAYS = 3  # 信号确认天数
...
# 信号确认（连续3天）
df['signal_confirm_count'] = df['base_signal'].groupby(
    (df['base_signal'] != df['base_signal'].shift()).cumsum()
).cumsum()
...
df.loc[
    (df['base_signal'] == 1) &
    (df['signal_confirm_count'] >= SIGNAL_CONFIRM_DAYS) &
    (df['volume_confirm']) &
    (df['trend_confirm']),
    'final_signal'
] = 1
...
trades_per_cycle = 50
cycles = min(10, len(df) // trades_per_cycle)
...
for cycle in range(cycles):
    ...
    print(f"  交易: {cycle_trades}次 (止损: {cycle_stop_loss_hits}次)")
...
# 止损统计
stop_loss_count = sum(1 for t in state['trades'] if t.get('type') == 'stop_loss')
daily_loss_count = sum(1 for t in state['trades'] if t.get('type') == 'daily_loss')
signal_count = sum(1 for t in state['trades'] if t.get('type') == 'signal')
...
print(f"  - 信号交易: {signal_count}次")
print(f"  - 止损: {stop_loss_count}次")
print(f"  - 日风控: {daily_loss_count}次")
...
if stop_loss_count > 0:
    avg_stop_loss = np.mean([t['pnl'] for t in state['trades'] if t.get('type') == 'stop_loss'])
    print(f"  平均止损幅度: {avg_stop_loss:+.2f}%")
else:
    print(f"  ✓ 无止损触发")

if daily_loss_count > 0:
    print(f"  日风控触发: {daily_loss_count}次")
else:
    print(f"  ✓ 无日风控触发")

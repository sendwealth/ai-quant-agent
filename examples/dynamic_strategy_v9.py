...
def get_trend_strength(df, i):
    ...
    # 趋势评分
    score = 0
    if price > ma_20: score += 0.3
    if ma_20 > ma_60: score += 0.3
    if ret_20 > 0.05: score += 0.2
    if ret_20 > 0.10: score += 0.2
    
    return min(score, 1.0)

def dynamic_backtest(df):
    """动态仓位回测"""
    df = df.copy()
    df['ma_s'] = sma(df['close'], 10)
    df['ma_l'] = sma(df['close'], 30)
    df['ema_s'] = ema(df['close'], 10)
    df['ema_l'] = ema(df['close'], 30)
    ...
    for i in range(60, len(df)):
        ...
        # 计算趋势强度
        trend = get_trend_strength(df, i)
        
        # 动态仓位：15%-45%
        current_pos_ratio = 0.15 + trend * 0.30
        ...
        # 买入信号：MA和EMA同时金叉
        signal_buy = ma_s > ma_l and ema_s > ema_l
        if signal_buy and shares == 0:
            new_shares = int(cash * current_pos_ratio / price)
            if new_shares > 0:
                shares = new_shares
                cash -= shares * price
                entry_price = price
                ...
                highest = price
                trades.append({'type': 'buy', 'price': price, 'pos': current_pos_ratio})
        
        # 卖出信号：死叉
        elif ma_s < ma_l and ema_s < ema_l and shares > 0:
            pnl = (price - entry_price) / entry_price
...
print("="*70)
print("📊 动态仓位策略 v9.0 (最终版)")
print("="*70)
print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("\n策略特点:")
print("  - 趋势强度决定仓位(15%-45%)")
print("  - MA+EMA双重确认")
print("  - 3倍ATR止损")
